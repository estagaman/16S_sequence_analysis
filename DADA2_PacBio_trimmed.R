...
#load fastq files 
path <- "/Users/elise/Downloads/PacBio_1/1985_fastq/test_set" #CHANGE TO LOCATION OF CCS FILES 
list.files(path)

#reading in names of fastq files 
fnFs <- sort(list.files(path, pattern=".fastq", full.names = TRUE)) #no reverse reads

sample.names <-basename(fnFs)

#REMOVE PRIMERS using CUTADAPT-------------------------------------------------------------------------------------

cutadapt <- path.expand("/Users/elise/opt/anaconda3/bin/cutadapt")
system2(cutadapt, args = "--version")

cut_dir <- file.path("/Users/elise/Downloads/PacBio_1/1985_fastq/test_set", "cutadapt")
...
hist(lens, 100)   #should all be slightly less than 1500
...
taxa <- assignTaxonomy(seqtab.nochim, "/Users/elise/Downloads/silva_nr99_v138.1_wSpecies_train_set.fa", multithread=TRUE)
